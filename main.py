...
def differential_evolution(F = 1, CR = 1, P = 50, n = None, bound = 100, numero_epoche = 1, exact = False):
    ...
    for i in range(0, P):
        population_fvalues[i] = local_search(x0 = population[:,i].reshape(n, 1), exact=exact)
    ...
    for epoca in range(0, numero_epoche):
        ...
        for i in range(0, P):
            ...
            trial_fvalue = local_search(x0 = trial, exact=exact)
...
differential_evolution(n = numVar, P=10, CR = 0, numero_epoche=20, exact=False, bound = 10) #ATTENZIONE AL BOUND, se test "quadratico" limitare!