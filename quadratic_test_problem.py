...
class QuadraticTestProblem(Funzione):
    ...
    def __init__(self, N=None, n = None, s = None, m = None, saveProblem = False, filename = ""):
        ...
        if saveProblem:
            with open('./temp_results/'+filename+"_problem", 'w') as file:
                temp_string = "A: \n"
                temp_string += str(self.A) + " \n"
                file.write(temp_string)
                temp_string = "x segnato:"
                temp_string+= str(self.x_segnato) + "\n"
                file.write(temp_string)

                temp_string = "Matrici Hi \n"
                for acca in self.HisPointer:
                    temp_string+= "\n" + str(acca) + "\n"
    ...
    def getQTauOttimoGivenYDE(self, tau, y, x0): #x0 è da dove inizia la ricerca
        #general implementation
        self.yTemp = y
        self.tauTemp = tau
        #print("y = " + str(self.yTemp) + " x0 = " + str(x0))
        #ottimo = minimize(self.qTauValueHelper, x0, method='BFGS')
        ottimo = optimize.differential_evolution(self.qTauValueHelper, optimize.Bounds(np.array(np.ones(y.shape[0])*(-5)), np.array(np.ones(y.shape[0])*(5))))
        #print(ottimo)
